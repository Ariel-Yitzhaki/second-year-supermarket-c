...
int initSuperMarket(SuperMarket* pMarket, const char* fileName,
	const char* customersFileName, int argc, char* argv[])
{
	...
	if (strcmp(argv[1], "1") == 0) // Use strcmp for string comparison
	{
		if (loadCompressedMarketFromFile(pMarket, fileName, customersFileName) == 1) {
			printf("Supermarket successfully loaded from files\n");
			return 1;
		}
	}
	else if (strcmp(argv[1], "0") == 0) // Use strcmp for string comparison
	{
	...
	// If we reach here, it means loading from files failed.
	printf("Error loading supermarket data from files. Please enter the market name.\n");
